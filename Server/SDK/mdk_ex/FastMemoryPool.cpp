...
void FastMemoryPool::Expand()
{
	...
	if ( NULL == buffer || NULL == pPool ) mdk_assert(false);//系统内存用完
...
void* FastMemoryPool::Alloc()
{
	...
	if ( NULL == pPool )
	{
		...
		if ( NULL == pPool ) mdk_assert(false);//系统内存用完

		index = AtomAdd(&m_threadCount, 1);
		m_poolIndex[index] = tid;
		m_poolForThread[index] = pPool;
	}
	mdk_assert( tid == pPool->m_ownerThreadId );//一出现并发立刻崩溃，确保其它地方的如果出错不用考虑并发可能
...
void FastMemoryPool::Free( void *pObject )
{
	...
	if ( 1 != AtomAdd(&pMemory->isAlloced, -1) )//捕获用户未配对的Alloc()/Free()调用
	{
		mdk_assert(false);
	}
	AtomAdd(&pMemory->pPool->freeCount, 1);
