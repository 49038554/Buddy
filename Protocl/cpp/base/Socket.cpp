...
//功能：等待数据
bool Socket::WaitData( int *list, int &count, long lSecond, long lMinSecond )
{
	if ( FD_SETSIZE < count ) return false;
	//接收超时设置
	timeval outtime;//超时结构
	outtime.tv_sec = lSecond;
	outtime.tv_usec =lMinSecond;
...
#ifdef WIN32
	FD_SET readfds;
	readfds.fd_count = count;
	for ( i = 0; i < readfds.fd_count; i++ ) readfds.fd_array[i] = list[i]; 
	//检查可读状态
	...
	for ( i = 0; i < count; i++ ) 
	{
		...
		if ( maxFd < list[i] ) maxFd = list[i];
	}
	//检查可读状态
	if ( lSecond < 0 && lMinSecond <= 0 ) 
	{
		nSelectRet=::select(maxFd+1, &readfds, NULL, NULL, NULL);
	}
	else 
	{
		nSelectRet=::select(maxFd+1, &readfds, NULL, NULL, &outtime);
	}
#endif
	if ( SOCKET_ERROR == nSelectRet ) 
	{
	...
	for ( i = 0; i < count; i++ ) 
	{
		if ( !FD_ISSET(list[i], &readfds) ) continue;
		list[nset] = list[i];
		nset++;
	}
	count = nset;
