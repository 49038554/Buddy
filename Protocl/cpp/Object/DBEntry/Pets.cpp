...
bool Pets::Build()
{
	...
	for ( i = 0; i < m_pets.size(); i++ )
	{
		...
		if ( !AddData(m_pets[i].HPHealthy) )//血
		{
			return false;
		}
		if ( !AddData(m_pets[i].WGHealthy) )//攻
		{
			return false;
		}
		if ( !AddData(m_pets[i].WFHealthy) )//防
		{
			return false;
		}
		if ( !AddData(m_pets[i].TGHealthy) )//特攻
		{
			return false;
		}
		if ( !AddData(m_pets[i].TFHealthy) )//特防
		{
			return false;
		}
...
bool Pets::Parse()
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( !GetData(pet.HPHealthy) )//血
		{
			return false;
		}
		if ( !GetData(pet.WGHealthy) )//攻
		{
			return false;
		}
		if ( !GetData(pet.WFHealthy) )//防
		{
			return false;
		}
		if ( !GetData(pet.TGHealthy) )//特攻
		{
			return false;
		}
		if ( !GetData(pet.TFHealthy) )//特防
		{
			return false;
		}
